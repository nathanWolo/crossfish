#!/usr/bin/env python3
"""Emit the packed D16/H8 MiniNet runtime header.
...
static int evaluate_mini_fast(const Board &b) {
    ...
    __m256i hidden = _mm256_load_si256(
        (const __m256i *)MN_FACTOR_INIT[c]);
    for (int mb = 0; mb < 9; mb++) {
        ...
        hidden = _mm256_add_epi32(
            hidden,
            _mm256_load_si256(
                (const __m256i *)MN_FACTOR_CODE[mb][code]));
        const int bit = 1 << mb;
        int super_cls = 0;
        ...
        if (super_cls) {
            hidden = _mm256_add_epi32(
                hidden,
                _mm256_load_si256(
                    (const __m256i *)MN_FACTOR_SUPER[mb][super_cls]));
        }
    }
    if (c < 9) {
        hidden = _mm256_add_epi32(
...
def emit_header(
    ...
    if d != 16 or h != 8:
        raise SystemExit(
            f"factored runtime is verified for D=16 H=8, got D={d} H={h}"
        )
    source = template_path.read_text(encoding="utf-8")
    marker = 'static const char MINI_PACK_B64[] = R"MNUE(\n'
    start = source.index(marker) + len(marker)
    end = source.index('\n)MNUE";', start)
    source = source[:start] + packed_b64 + source[end:]
    ...
    source = source.replace("MINI_PACK_B64", "D16_MINI_PACK_B64")
    source = re.sub(r"\bMN_", "D16_MN_", source)
    for name in (
        ...
        source = re.sub(rf"\b{name}\b", f"d16_{name}", source)
...
def main() -> None:
    ...
    parser.add_argument("net", type=Path, help="D16/H8 CFM2 checkpoint")
    parser.add_argument(
        "-o",
        "--out",
        type=Path,
    ...
    emit_header(args.template, args.out, packed_b64, d, h)
